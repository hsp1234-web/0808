...
### fix(core): 修正 Gemini 模型列表載入時 API 金鑰傳遞的穩定性問題

- **動機**: 使用者回報，在前端輸入 API 金鑰後，載入 Gemini 模型列表時偶爾會出現「API 金鑰遺失」的錯誤，即使大部分功能看起來正常。這表明前端到後端再到工具腳本的金鑰傳遞鏈路存在不穩定的環節。
- **根本原因分析**:
    - 經過深入追查，確認前端 (`mp3.html`) 和後端 API 伺服器 (`api_server.py`) 的金鑰傳遞邏輯皆為正確。
    - 問題的根源被定位在最終執行的工具腳本 `src/tools/gemini_processor.py` 中。該腳本內的 `list_models` 函式在處理「找不到 API 金鑰」這一特定錯誤時，其錯誤處理機制不夠明確。它拋出了一個通用的 `Exception`，導致上層的 `api_server.py` 無法準確判斷失敗原因，從而回傳給前端一個模糊的錯誤訊息。
- **核心變更**:
    - **強化錯誤處理 (`gemini_processor.py`)**:
        - 修改了 `list_models` 函式。
        - 新增了與 `validate_key` 函式一致的、更健壯的檢查邏輯。現在，如果 `GOOGLE_API_KEY` 環境變數未被設定，腳本會立即向 `stderr` 印出一條明確、可被機器識別的錯誤訊息 `API Key not found in environment variables.`，然後以失敗狀態碼 `1` 退出。
- **成果**: 此次修復確保了當金鑰遺失時，後端服務鏈路上的每個環節都能收到一個清晰、一致的失敗信號。這使得 `api_server.py` 能夠正確地捕捉此特定錯誤，並將準確的「API 金鑰遺失」提示回傳給前端，從而徹底解決了此問題，並提升了系統的整體健壯性和可除錯性。
