## 2025-08-31T14:21:45+08:00

### fix(core): 修正媒體下載與 AI 模型載入的核心邏輯 (最終版)

- **動機**: 使用者提報兩個獨立的後端問題：1) 媒體下載功能因寫死的 `yt-dlp` 路徑而完全失敗。 2) AI 模型列表因 API 金鑰未在請求間正確傳遞而無法載入。
- **核心變更**:
    - **修復媒體下載路徑 (最終方案)**:
        - **檔案**: `src/tools/youtube_downloader.py`
        - **問題**: 初步修復為直接呼叫 `"yt-dlp"`，但在某些環境 (如 Colab) 中，可執行檔未被加入系統 `PATH`，導致 `FileNotFoundError`。
        - **最終修復**: 改用 `[sys.executable, "-m", "yt_dlp", ...]` 的方式呼叫。此方法透過 Python 模組來執行 `yt-dlp`，不依賴系統 `PATH`，確保在任何已安裝該套件的 Python 環境中都能運作，更具可攜性與穩健性。
    - **修復 API 金鑰傳遞邏輯 (短期方案)**:
...
- **驗證**:
    ...
    - **媒體下載驗證**: 測試確認了新的模組化呼叫方式能正確執行，並能處理其回傳的網路錯誤 (預期中的 403 Forbidden)，證明了路徑修復成功。
