## 2025-08-30T07:25:00+08:00

### feat(launcher): 升級 Colab 啟動器並建立 `colabPro.py`

- **動機**: 為了整合最新、最穩定的啟動器功能，並為未來的主要啟動器建立一個清晰的基礎。
- **核心變更**:
    - **功能移植**: 將一個更先進的啟動器版本（「善狼啟動器 v17.0」）中的核心功能，成功移植到現有的 `scripts/colab.py`。
        - **併發通道管理器 (`TunnelManager`)**: 引入了新的多執行緒通道管理器，能夠同時嘗試連接 Cloudflare, Localtunnel, 和 Colab 官方代理，並列出所有成功的網址，大幅提升了連線的穩定性和速度。
        - **HTML 日誌報告**: 新增了在腳本結束時生成可互動、可複製 HTML 報告的功能，方便使用者儲存和分析日誌。
    - **保留優點**: 在移植新功能的同時，根據使用者要求，完整地保留了舊版本中將日誌歸檔至 `paper` 資料夾的 `archive_reports` 功能，實現了新舊設計的結合。
    - **建立 `colabPro.py`**:
        - 將功能升級後的 `scripts/colab.py` 複製到專案根目錄，並命名為 `colabPro.py`，作為未來主要的 Colab 入口點。
        - 將 `colabPro.py` 的內部版本號更新至 `v18`，並在檔案內的開發者日誌中詳細記錄了所有變更。
- **成果**: 成功打造了一個集兩者之長的新版 Colab 啟動器 `colabPro.py`，它既擁有先進的網路連接能力和 UI，也保留了經過驗證的、穩定的日誌歸檔邏輯。
