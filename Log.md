## 929號 - 2025-09-02T11:16:57.265353+08:00

### fix(core): 全面修復後端穩定性與前端渲染鏈

- **動機**: V3 版本重構後，應用程式處於完全不可用狀態。後端處理特定音訊時會崩潰，且前端因 JavaScript 錯誤導致頁面無法渲染。本次修復旨在恢復應用程式的核心功能。
- **核心變更**:
    - **後端穩定性修復 (`src/services/transcriber.py`)**:
        - **問題**: `faster-whisper` 函式庫無法處理具有奇數樣本點的音訊檔，導致在轉錄極短音訊時引發 C++ 層級的斷言錯誤，使整個伺服器崩潰。
        - **修復**: 在將音訊數據傳遞給模型前，新增了一個檢查步驟。如果音訊樣本數為奇數，則自動填充一個零樣本，將其變為偶數，從而繞過底層函式庫的崩潰點。
    - **前端渲染鏈修復 (`src/static/templates/base.html`)**:
        - **問題**: 前端頁面一片空白，Playwright 測試顯示 `bootstrap.Tab is not a constructor` 的 JavaScript 錯誤。
        - **修復**: 經查，`base.html` 模板中遺失了對 Bootstrap 核心 JS 檔案的引入。在 `app.js` 之前加入了 `<script>` 標籤來載入 `bootstrap.bundle.min.js`，解決了 JS 執行錯誤，使頁面得以正常渲染。
- **測試**:
    - **後端**: 透過自動化腳本反覆測試，證明了後端在處理先前導致崩潰的音訊檔時，能夠穩定完成任務而不再崩潰。
    - **前端**: 編寫了 `verify_buttons.py` Playwright 腳本，透過安裝缺失的系統依賴 (`playwright install-deps`) 和修復 JS 錯誤，最終成功執行。腳本驗證了三種任務類型（本地、YouTube、下載）都能被正確建立，且其對應的操作按鈕都已在 UI 上成功還原。
- **成果**: 本次提交透過對後端穩定性和前端渲染鏈的雙重修復，成功將應用程式從完全癱瘓的狀態中恢復過來，使其核心功能（任務提交、處理、UI 互動）恢復正常，為後續的功能驗證鋪平了道路。
