## 920號 - 2025-09-02T12:12:38.837733+08:00

### test(e2e): 建立 `mp3.html` 的基準 Playwright 測試

- **動機**: 為了輔助未來的前端架構重構，需要一個穩定、簡單的端對端 (E2E) 測試作為基準。此測試將用於快速驗證最核心的頁面載入與 UI 渲染是否正常。
- **核心變更**:
    - **建立新測試檔案 (`src/tests/e2e_baseline_mp3.spec.js`)**:
        - 新增了一個 Playwright 測試套件，專門用於 `mp3.html` 的初始狀態驗證。
        - 測試腳本會導覽至頁面，並斷言 (assert) WebSocket 連線成功 (狀態顯示「已連線」)，以及三個主要的功能分頁按鈕 (`本機檔案轉錄`, `媒體下載器`, `YouTube 轉報告`) 都已正確顯示。
    - **除錯與修正**:
        - 在初次執行時，測試因 `cross-env` 指令未找到而失敗。透過直接呼叫 `node_modules` 中的執行檔路徑 (`./node_modules/.bin/cross-env`) 解決了此環境問題。
        - 接著，測試因定位器 (locator) 錯誤而再次失敗。經檢查 `mp3.html` 原始碼，修正了測試腳本中不正確的分頁按鈕 `data-tab` 屬性，使其與實際 UI 相符。
- **測試**:
    - 成功執行了新建的 `e2e_baseline_mp3.spec.js` 測試。
    - 測試通過後，自動產生了 `e2e_baseline_success.jpg` 螢幕截圖作為成功證明。
- **成果**: 成功建立了一個可靠的前端基準測試。這個測試未來可以用於迴歸測試 (regression testing)，確保任何對 `mp3.html` 的修改都不會破壞其最基本的載入功能。
