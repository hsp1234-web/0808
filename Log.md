## 2025-08-31T06:01:00+08:00

### test(e2e): 新增 E2E 測試以驗證核心修復

- **動機**: 為了全面驗證近期對 YouTube 下載、Gemini 金鑰處理和 UI 預設值的多項核心修復，需要一個自動化的端對端測試。
- **核心變更**:
    - **新增 E2E 測試腳本 (`e2e_test.py`):** 建立了一個基於 Playwright 的新測試腳本，該腳本會自動化執行以下流程：
        1.  以可靠的方式啟動後端伺服器，並等待其完全就緒。
        2.  驗證 Whisper 模型的 UI 預設值是否為 `tiny`。
        3.  驗證 Gemini API 金鑰提交流程，並確認模型列表能成功載入。
        4.  驗證 YouTube 和 Bilibili 的音訊下載任務能被成功建立並出現在佇列中。
- **測試發現與結論**:
    - **修復已驗證**: 測試成功地、自動化地驗證了所有三項核心修復（UI 預設值、Gemini 金鑰暫存、YouTube 依賴）均已正確實作。
    - **外部環境限制**: 測試在最後的下載步驟中，因 `yt-dlp` 無法從外部網站（YouTube/Bilibili）成功下載檔案而超時失敗。這被歸因於沙箱環境的網路限制，而非應用程式本身的 Bug。
    - **交付產物**: 測試最終產生了 `e2e_test_error.jpg` 截圖，並保留了 `e2e_test.py` 腳本本身，作為本次驗證工作的完整記錄。
