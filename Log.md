## 923號 - 2025-09-04T22:55:04.396134+08:00

### fix(core): 修復多項前端顯示與後端通訊錯誤並提交交接報告

- **動機**: 在對 MPA 重構進行驗證時，發現了數個導致 E2E 測試必定失敗的嚴重 Bug。在修復了表層問題後，核心功能依然無法通過測試。應使用者要求，停止深入除錯，轉而記錄所有發現並提交一份詳細的交接報告。
- **核心變更**:
    - **修復 MPA 頁面內容顯示問題**:
        - **問題**: MPA 的 `downloader.html` 和 `youtube.html` 頁面內容區塊缺少 `active` class，導致頁面載入時功能區不可見。
        - **修復**: 為上述兩個檔案的 `.tab-content` `div` 補上了 `active` class。
    - **修復全域儀表板數據更新問題**:
        - **問題**: 儀表板的 CPU/RAM 數據在所有頁面都顯示為 `--%`。
        - **修復**: 將 `index.html` 中負責更新儀表板的 `updateSystemStats` JavaScript 函式，複製並添加到了 `downloader.html` 和 `youtube.html` 中，確保所有頁面都能正確更新數據。
    - **修復 API 金鑰驗證環境問題**:
        - **問題**: 即便提供了有效的 API 金鑰，前端驗證依然失敗。
        - **修復**: 經調查發現是 `api_server.py` 在呼叫驗證腳本的子程序時，未傳遞 `PYTHONPATH` 環境變數。已在 `validate_api_key` 函式中將 `PYTHONPATH` 加入子程序的執行環境。
    - **建立交接報告 (`HANDOVER_PLAN_V2.md`)**:
        - 撰寫了一份詳細的報告，記錄了所有已修復的 Bug、當前 E2E 測試的失敗狀態，以及對剩餘底層問題（很可能與 `asyncio` 和 `subprocess` 有關）的分析與除錯建議。
- **測試**:
    - 前端顯示問題已透過基準螢幕截圖測試驗證並修復。
    - 核心的綜合 E2E 測試 (`e2e_comprehensive_real.spec.cjs`) **仍然全部失敗**。失敗原因已在交接報告中詳細記錄。
- **成果**: 本次提交修復了所有已知的、清晰的前端顯示層和部分後端通訊配置的 Bug。雖然核心功能依然存在問題，但程式碼庫的狀態已得到改善，並且提供了一份詳盡的交接報告，為後續解決這個頑固的底層錯誤鋪平了道路。
