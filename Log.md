## 928號 - 2025-09-02T10:21:11.929071+08:00

### fix(core): 全面修復前端 UI 與後端處理流程

- **動機**: 使用者回報，在 V3 前端重構後，應用程式的多項核心功能完全失效。這包括已完成任務的操作按鈕遺失、本地轉錄的模型下載功能失效、YouTube 報告的金鑰驗證與模型列表載入失敗。此外，後端在處理影片下載時會無限期掛起，導致整個任務佇列癱瘓。
- **核心變更**:
    - **前端修復 (`transcribe.html`, `youtube_report.html`, `app.js`)**:
        - **恢復 UI 元素**: 在 `transcribe.html` 中重新加入了「確認設定」按鈕和模型下載進度條的 HTML。
        - **恢復 UI 邏輯**:
            - 為 `transcribe.html` 重新實作了模型下載的 JavaScript 邏輯，使其能透過 WebSocket 正確觸發後端下載。
            - 為 `youtube_report.html` 恢復了 API 金鑰驗證、模型列表載入、以及基於金鑰狀態啟用/禁用相關控制項的完整 JavaScript 流程。
            - 全面修正了共享腳本 `app.js` 中的 `getTaskHtml` 函式，確保所有任務類型（報告、轉錄、下載）在完成後都能顯示其對應的完整操作按鈕（詳細資料、預覽、下載等）。
    - **後端修復 (`youtube_downloader.py`)**:
        - **解決掛起問題**: 在呼叫 `yt-dlp` 的 `subprocess.run` 中，新增了 120 秒的 `timeout` 參數。這項關鍵修復解決了後端程序在下載特定影片（如 YouTube Shorts）時無限期掛起的問題，確保了任務佇列的穩定性。
- **測試**:
    - **端到端測試**: 經過後端修復後，成功執行了完整的端到端測試。測試涵蓋了：
        - 使用者提供的 Gemini API 金鑰和 YouTube 影片網址，驗證了從金鑰儲存、模型選擇、任務提交到最終報告按鈕呈現的完整流程。
        - 上傳模擬音訊檔 (`test_audio.mp3`)，驗證了本地轉錄功能的模型下載、轉錄過程和最終按鈕呈現。
    - **Playwright 截圖驗證**:
        - 建立了一個全面的 Playwright 腳本，透過注入模擬資料和直接與修復後的 UI 互動，截取了所有核心頁面的螢幕截圖，以供使用者審閱。
- **成果**: 本次提交不僅徹底修復了使用者回報的所有前端 UI 錯誤，還診斷並解決了一個潛在的、更為嚴重的後端掛起問題。應用程式的核心功能現已完全恢復，並且更加健壯和可靠。
