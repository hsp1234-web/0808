...
## 前置作業：使用快照腳本進行環境驗證 (推薦)

在進行任何深入的 E2E 或單元測試之前，強烈建議先執行輕量級的快照腳本來驗證您的本地開發環境是否已正確設定。這個腳本是解決本報告後續章節中提到的許多環境問題的最終成果。
...
**目的:**
*   **一鍵設定**: 此指令會自動安裝所有 Python 依賴、Node.js 依賴，並下載測試所需的瀏覽器。
*   **快速驗證**: 它會啟動伺服器，擷取一張首頁快照，然後自動關閉。
*   **避免挫折**: 如果此腳本成功執行，代表您的環境已準備就緒，可以進行更複雜的測試。如果失敗，其日誌通常能提供比完整 E2E 測試更直接的錯誤線索。

**請將執行此腳本作為您開始任何測試工作的第一步。**
