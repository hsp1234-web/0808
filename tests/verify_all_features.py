...
def run_e2e_test():
    ...
    try:
        with sync_playwright() as p:
            ...
            # 驗證順向排序：檢查最後一個 <p> 元素是否包含模擬腳本的最後一句話
            last_sentence = "轉錄即將完成。"
            expect(p_elements.last).to_contain_text(last_sentence)

            print("✅ 轉錄結果順向排序與顯示驗證成功。")

            print("▶️ 驗證日誌查看器位置與功能...")
            # 根據使用者要求，驗證日誌查看器現在緊跟在轉錄輸出之後
            expect(page.locator("#transcript-container + #log-viewer-card")).to_be_visible()
