...
test.describe("鳳凰音訊轉錄儀 E2E 整合測試", () => {
  ...
  test("本地檔案轉錄 - 完整流程驗證", async ({ page }) => {
    await page
      .locator("#file-input")
      .setInputFiles([DUMMY_FILE_NAME_1, DUMMY_FILE_NAME_2]);
    const fileList = page.locator("#file-list");
    await expect(
      fileList.locator(".task-item", { hasText: DUMMY_FILE_NAME_1 }),
    ).toBeVisible();
    const fileItem2 = fileList.locator(".task-item", {
      hasText: DUMMY_FILE_NAME_2,
    });
    await expect(fileItem2).toBeVisible();
    await fileItem2.locator('button:has-text("移除")').click();
    await expect(
      fileList.locator(".task-item", { hasText: DUMMY_FILE_NAME_2 }),
    ).not.toBeVisible();
    ...
    const ongoingTask = page.locator("#ongoing-tasks .task-item", {
      hasText: DUMMY_FILE_NAME_1,
    });
    await expect(ongoingTask).toBeVisible();
    await expect(ongoingTask.locator(".task-status")).toContainText(
      "轉錄中...",
      { timeout: 15000 },
    );
    ...
    await expect(iframe.locator("pre")).toContainText(MOCK_TRANSCRIPT_TEXT, {
      timeout: 10000,
    });
