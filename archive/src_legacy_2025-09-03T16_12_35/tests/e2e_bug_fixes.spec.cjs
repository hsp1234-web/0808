...
// 測試資料
const YOUTUBE_URL = 'https://youtube.com/shorts/HMS4V6VTIaI?si=UgJD_EiARGXb15VX';
...
test.describe('錯誤修復驗證測試 (Bug Fix Verification Tests)', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto('/');
    // 等待 WebSocket 連線成功
    await expect(page.locator('#status-text')).toHaveText('已連線', { timeout: 15000 });
  });

  test('【驗證】媒體下載功能：移除寫死路徑後應能正確回報網路錯誤', async ({ page }) => {
    // 1. 切換到「媒體下載器」分頁
    ...
    await expect(taskElement.locator('.task-filename')).toContainText('youtube.com');
  ...
  test('【驗證】AI 模型載入功能：API 金鑰應能正確傳遞並載入模型', async ({ page }) => {
    ...
    // 4. 斷言：UI 顯示金鑰有效
    await expect(page.locator('#api-key-status > span')).toHaveText('金鑰有效，Gemini 功能已啟用', { timeout: 10000 });

    // 5. 關鍵步驟：等待模型列表的 API 請求完成，徹底消除競爭條件
    await page.waitForResponse(resp => resp.url().includes('/api/youtube/models') && resp.status() === 200, { timeout: 10000 });

    // 6. 斷言：「AI 模型」下拉式選單成功載入模型
    const modelSelect = page.locator('#gemini-model-select');

    // 現在可以安全地進行斷言
    const firstOptionText = await modelSelect.locator('option').first().textContent();
