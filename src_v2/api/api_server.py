...
# JULES MPA Refactor: Add a generic route to serve other top-level HTML pages.
@app.get("/{page_name}.html", response_class=HTMLResponse)
async def serve_html_pages(request: Request, page_name: str):
    """
    提供 `static` 目錄下的頂層 HTML 檔案，例如 /transcribe.html。
    這是支援 MPA 架構的關鍵。
    """
    # 組成檔案路徑
    html_file_path = STATIC_DIR / f"{page_name}.html"
    ...
    # 如果檔案不存在，回傳 404
    log.warning(f"請求的 HTML 頁面不存在: {html_file_path}")
    raise HTTPException(status_code=404, detail=f"找不到頁面: {page_name}.html")
