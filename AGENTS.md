...
## Special Directive for v2 E2E Timeout Issue (JULES, 2025-09-04)

**Problem:** The full E2E test suite (`npx playwright test --config=playwright.config.v2.js`) is currently hanging and timing out after ~400 seconds. However, every single test file runs to completion quickly when executed individually.

**Hypothesis:** This points to a resource leak or race condition in the backend server (`api_server_v2.py`) that only manifests under the high load of the full test suite.

**Action Plan for the Next Agent:**

Your primary objective is to diagnose and fix this timeout issue. Do not attempt to fix individual test failures until the full suite can run to completion without hanging.

1.  **Instrument with Heartbeats:** Your first step should be to instrument a complex test file with "heartbeat" logs to pinpoint the exact operation that is hanging. See `HANDOVER_PLAN.md` for a detailed guide and example. This is the highest priority.

    ```javascript
    // Example of a heartbeat log
    console.log("[Heartbeat] Step 3: Clicking submit...");
    await page.locator("#submit-btn").click();
    console.log("[Heartbeat] Submit clicked.");
    ```

2.  **Review Backend Code:** Once the hanging operation is identified (e.g., a specific API call), meticulously review the corresponding Python code in `api_server_v2.py`. Look for:
    - Unclosed file handles (`with open(...)` should be used).
    - Mishandled subprocesses (`subprocess.run` or `Popen`).
    - Potential deadlocks in async code.

3.  **Do Not Get Distracted:** You will see many individual tests failing for various reasons (wrong text, missing elements). **Ignore them.** The only goal is to make the full test suite run to completion without a global timeout. Once the suite runs, then you can proceed to fix the individual failures.
