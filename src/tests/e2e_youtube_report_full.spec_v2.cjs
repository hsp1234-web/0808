...
test.describe('完整 YouTube 報告生成流程測試', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // 等待核心系統就緒
    await expect(page.locator('#status-text')).toHaveText('已連線', { timeout: 30000 });
  });
  ...
  test('應能成功處理多個 URL、驗證所有 UI 元素並產生報告', async ({ page }) => {
    ...
    await expect(ongoingTasksContainer.locator('.task-filename').getByText(/mG5z-pfhIiA/).first()).toBeVisible({ timeout: 20000 });
    await expect(ongoingTasksContainer.locator('.task-filename').getByText(/xALqLQM/).first()).toBeVisible({ timeout: 20000 });

    // 驗證狀態文字包含「處理中」
    // 這裡我們只驗證第一個任務的狀態，作為代表
    await expect(ongoingTasksContainer.locator('.task-item .task-status').first()).toContainText('處理中', { timeout: 20000 });

    // --- 6. 等待任務完成並驗證結果 ---
    const reportBrowser = page.locator('#youtube-file-browser');

    // 等待報告出現在瀏覽區，給予足夠長的超時時間
    await expect(reportBrowser.locator('.task-item')).toHaveCount(2, { timeout: 4 * 60 * 1000 });
    ...
    // 對第一個報告進行詳細驗證
    await expect(firstReport.getByRole('link', { name: '詳細資料' })).toBeVisible();
    ...
    await expect(firstReport.getByRole('link', { name: '修改名稱' })).toBeVisible();
    ...
    // 測試全部通過的最終斷言
    await expect(page.locator('#no-ongoing-task-msg')).toBeVisible({ timeout: 10000 });
  });
