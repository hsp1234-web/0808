...
const expectStatusMessage = async (page, expectedMessage, isError = false) => {
  ...
  await expect(page.locator("#status-message-text")).toHaveText(
    expectedMessage,
  );
  if (isError) {
    await expect(statusMessageArea).toHaveCSS(
      "background-color",
      "rgb(248, 215, 218)",
    );
  }
...
test.describe("綜合功能 E2E 測試", () => {
  ...
  test("應能成功提交 YouTube 下載任務", async ({ page }) => {
    ...
    const taskText = await taskList
      .locator(".task-item .task-filename")
      .textContent();
  ...
  test("應能使用 Base64 成功上傳本地檔案", async ({ page }) => {
    ...
    await expect(page.locator("#file-list .task-filename")).toHaveText(
      "dummy_audio.wav",
    );
    ...
    await expect(ongoingTasks.locator(".task-item")).toHaveCount(1, {
      timeout: 10000,
    });
    const taskText = await ongoingTasks
      .locator(".task-item .task-filename")
      .textContent();
