...
const JpgScreenshot = async (page, name) => {
  ...
  await page.screenshot({
    path: path.join(resultsDir, `${name}.jpg`),
    type: "jpeg",
  });
};
...
// V2 REFACTOR: The hardcoded BASE_URL has been removed. Tests now use relative paths.
test.describe.skip("AI Visual Patrol and Core Functionality Test", () => {
  test.beforeEach(async ({ page }) => {
    ...
    page.on("requestfailed", (request) =>
      console.log(
        `[Network Fail] ${request.url()} ${request.failure()?.errorText}`,
      ),
    );
    // Listen and log page errors
    page.on("pageerror", (exception) =>
      console.log(`[Page Error] Unhandled exception: ${exception}`),
    );
  });
  ...
  test("Step 2: MP3 Page Patrol", async ({ page }) => {
    await page.goto("/static/mp3.html", { waitUntil: "networkidle" });
  ...
  test("Step 3: Prompts Page Patrol", async ({ page }) => {
    await page.goto("/static/prompts.html", { waitUntil: "networkidle" });
  ...
  test("Step 5: Upload Audio and Verify Transcription", async ({ page }) => {
    ...
    const filePath = path.join(__dirname, "fixtures", "test_audio_v2.mp3");
