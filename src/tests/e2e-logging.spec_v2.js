...
const expectLatestLogToContain = async (page, expectedMessage) => {
  await expect(async () => {
    ...
    const response = await page.request.get(
      "/api/debug/latest_frontend_action_log",
    );
    ...
    expect(
      json.latest_log.message,
      `最新的日誌訊息不包含 "${expectedMessage}"`,
...
test.describe("前端操作日誌 E2E 測試 (資料庫模式)", () => {
  ...
  test("應能正確將各種 UI 互動記錄到資料庫", async ({ page }) => {
    ...
    await test.step("測試按鈕點擊", async () => {
      // This button is on the main page ('/')
      await page.locator("#confirm-settings-btn").click();
      await expectLatestLogToContain(page, "click-confirm-settings: medium");
    });

    // 2. 測試下拉選單變更 (Select Change)
    await test.step("測試下拉選單變更", async () => {
      // V2 REFACTOR: Navigate directly to the main page to find the model select.
      await page.goto("/", { waitUntil: "domcontentloaded" });
      await page.locator("#model-select").selectOption("large-v3");
      await expectLatestLogToContain(page, "change-model-select: large-v3");
    });

    // 3. 測試核取方塊 (Checkbox Toggle)
    await test.step("測試核取方塊", async () => {
      // V2 REFACTOR: Navigate directly to the downloader page.
      await page.goto("/downloader_v2.html", { waitUntil: "domcontentloaded" });

      // Ensure the checkbox exists on this page before interacting.
      const checkbox = page.locator("#remove-silence-checkbox");
    ...
    await test.step("測試日誌檢視器篩選器", async () => {
      // V2 REFACTOR: Navigate back to the main page to find the log viewer.
      await page.goto("/", { waitUntil: "domcontentloaded" });
      const filterCheckbox = page.locator(
        'input[name="log-level"][value="ERROR"]',
      );
