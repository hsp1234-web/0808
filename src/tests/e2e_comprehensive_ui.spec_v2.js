...
test.describe('綜合性 UI 功能測試: mp3.html', () => {
  ...
  test.describe('「本機檔案轉錄」分頁功能測試', () => {
    test('應能成功上傳檔案並擷取分頁截圖', async ({ page }) => {
      ...
      await page.screenshot({ path: 'mpa_design_references/01_local_file_task_created.jpg', fullPage: true });

      // 6. 等待任務完成並驗證
      const completedTask = page.locator('#completed-tasks .task-item');
      await expect(completedTask).toContainText('mock_upload.txt', { timeout: 15000 });
      await expect(ongoingTask).not.toBeVisible(); // 驗證任務已從處理中列表移除
      await page.screenshot({ path: 'mpa_design_references/02_local_file_task_completed.jpg', fullPage: true });

      // 7. 驗證預覽功能
      // API 端點註解: 預覽按鈕會讀取任務結果中的 output_path，並在前端開啟一個 Modal 來顯示內容，不直接呼叫後端 API。
      const previewBtn = completedTask.locator('[data-testid="view-report-button"]');
      await previewBtn.click();
      const previewModal = page.locator('#preview-modal');
      await expect(previewModal).toBeVisible();
      // 驗證 Modal 內的內容 (mock server 應回傳特定文字)
      await expect(previewModal.locator('.modal-body pre')).toContainText('這是模擬的轉錄稿內容');
      await page.screenshot({ path: 'mpa_design_references/03_local_file_preview_modal.jpg', fullPage: true });
    });
  });
  ...
  test.describe('「媒體下載器」分頁功能測試', () => {
    test('應能成功提交下載任務並擷取分頁截圖', async ({ page }) => {
      ...
      await page.screenshot({ path: 'mpa_design_references/04_downloader_task_created.jpg', fullPage: true });

      // 5. 等待任務完成並驗證
      // Mock server 應該很快完成這個任務
      await expect(taskItem.locator('[data-testid="view-report-button"]')).toBeVisible({ timeout: 15000 });
      await expect(taskItem).toContainText('已完成'); // 驗證狀態文字
      await page.screenshot({ path: 'mpa_design_references/05_downloader_task_completed.jpg', fullPage: true });
    });
  });

  // --- 測試案例 3: 「YouTube 轉報告」分頁 ---
  test.describe('「YouTube 轉報告」分頁功能測試', () => {
    test('應能成功儲存 API Key、提交分析任務、並擷取分頁截圖', async ({ page }) => {
      // 1. 點擊「YouTube 轉報告」分頁
      // API 端點註解: 純前端互動
      ...
      await expect(page.locator('[data-testid="gemini-model-select"]')).toHaveValue('gemini-pro-mock');
      ...
      await page.screenshot({ path: 'mpa_design_references/06_youtube_task_created.jpg', fullPage: true });
      ...
      await page.screenshot({ path: 'mpa_design_references/07_youtube_report_item.jpg', fullPage: true });

      // 6. 驗證預覽功能
      const previewBtn = reportItem.locator('[data-testid="view-report-button"]');
      await previewBtn.click();
      const previewModal = page.locator('#preview-modal');
      await expect(previewModal).toBeVisible();
      ...
      await page.screenshot({ path: 'mpa_design_references/08_youtube_report_preview.jpg', fullPage: true });
    });
  });
