...
test.describe('綜合性端對端修復驗證', () => {
  ...
  test('步驟 1: 媒體預覽 - 應能成功下載並預覽帶有特殊字元的影片', async ({ page }) => {
    // 導航到媒體下載器分頁
    ...
    const previewButtonForId = taskElement.locator('a.btn-preview:has-text("預覽")');
    downloadedTaskId = await previewButtonForId.getAttribute('data-task-id');
    expect(downloadedTaskId, '無法從預覽按鈕的 data-task-id 屬性中提取 task-id').toBeTruthy();
    ...
    await expect(audioPlayer).toHaveJSProperty('networkState', 1, { timeout: 15000 });
  ...
  test('步驟 2: Gemini 模型 - 應能成功載入模型列表', async ({ page }) => {
    ...
    // 驗證金鑰狀態是否變為有效
    await expect(page.locator('#api-key-status')).toContainText('金鑰有效', { timeout: 15000 });

    // 驗證模型下拉選單是否成功載入
    const modelSelect = page.locator('#gemini-model-select');
    // 等待第一個選項出現，並確保它不是錯誤訊息
    await expect(modelSelect.locator('option').first()).not.toContainText('載入失敗', { timeout: 15000 });
    await expect(modelSelect.locator('option').first()).not.toContainText('等待從伺服器載入', { timeout: 15000 });
  ...
  test('步驟 3: 檔案重新命名 - 應能成功重新命名已下載的檔案', async ({ page }) => {
    expect(downloadedTaskId, '前一個測試未成功設定 downloadedTaskId').not.toBeNull();

    // 重新導航到包含已完成任務的頁面，確保狀態一致
    await page.click('button[data-tab="youtube-report-tab"]');

    const taskElement = page.locator(`#completed-tasks .task-item:has-text("#貓meme 搞笑影片")`);
    ...
    await expect(page.locator(newFilenameSelector)).toBeVisible({ timeout: 10000 });

    // 驗證舊的檔名已不存在
    await expect(page.locator(`#completed-tasks .task-item:has-text("#貓meme 搞笑影片")`)).not.toBeVisible();
  });
});
