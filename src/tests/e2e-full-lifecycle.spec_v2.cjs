...
test.describe('Full Task Lifecycle E2E Test', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/system_stats', route => {
      route.fulfill({ status: 200, json: { cpu_usage: 15.5, ram_usage: 55.2, gpu_detected: true, gpu_usage: 25.8 } });
    });
    ...
    await page.route('**/api/log/action', route => route.fulfill({ status: 200 }));
  ...
  test('should correctly display dashboard, process a task, and show results with buttons', async ({ page }) => {

    // --- 1. Verify Dashboard ---
    await expect(page.locator('#cpu-label')).not.toContainText('--%', { timeout: 10000 });
    await expect(page.locator('#ram-label')).not.toContainText('--%', { timeout: 10000 });
    await expect(page.locator('#gpu-label')).not.toContainText('--%', { timeout: 10000 });
    await page.screenshot({ path: 'test-results/lifecycle-step1-dashboard-ok.jpg' });
    console.log('✅ [Test Step 1/5] Dashboard stats loaded correctly.');

    // --- 2. Start a Task ---
    // Use a real file for the upload to make the test more realistic
    const filePath = 'src/tests/fixtures/test_audio_v2.mp3';
    await page.setInputFiles('input#file-input', filePath);
    await expect(page.locator('#file-list .task-filename')).toContainText('test_audio_v2.mp3');
    ...
    await page.screenshot({ path: 'test-results/lifecycle-step2-processing-with-timer.jpg' });
    ...
    const mockResult = {
        ...
        original_filename: 'test_audio_v2.mp3',
    ...
    await expect(completedTaskItem).toContainText('test_audio_v2.mp3');
    ...
    const downloadButton = completedTaskItem.getByRole('link', { name: '下載' });
    await expect(previewButton).toBeVisible();
    await expect(downloadButton).toBeVisible();
    await page.screenshot({ path: 'test-results/lifecycle-step3-completed-with-buttons.jpg' });
    ...
    await page.route('**/media/mock-results/test_audio.txt', route => {
        route.fulfill({ status: 200, contentType: 'text/plain', body: 'This is the mock transcript content.' });
    });

    await previewButton.click();
    const modal = page.locator('#preview-modal');
    await expect(modal).toBeVisible();
    await expect(modal.locator('.modal-body pre')).toContainText('This is the mock transcript content.');
    await page.screenshot({ path: 'test-results/lifecycle-step4-preview-works.jpg' });
