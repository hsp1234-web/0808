...
test.describe("Layout and Error Handling Before Fixes", () => {
  ...
  const MOCK_TASKS = [
    ...
    },
    {
      ...
      payload: {
        original_filename:
          "this-is-a-very-very-very-long-filename-that-is-designed-to-break-the-layout-of-the-task-list-and-test-the-ellipsis-feature.mp3",
      },
      result: null,
    },
    {
  ...
  test.beforeEach(async ({ page }) => {
    // V2 REFACTOR: Clear tasks to prevent state pollution, even though we mock the next call.
    ...
    await page.route("**/api/system_stats", (route) =>
      route.fulfill({
        status: 200,
        json: { cpu_usage: 10, ram_usage: 50, gpu_detected: false },
      }),
    );
  });
  ...
  test("should capture layout issues on Desktop", async ({ page }) => {
    ...
    // V2 REFACTOR: Navigate directly to the page instead of clicking a tab.
    ...
    await page
      ...
      .fill("https://dummy.url/for/testing");
    await page
      .getByTestId("start-youtube-processing-button")
      .click({ force: true }); // Force click as it might be disabled
