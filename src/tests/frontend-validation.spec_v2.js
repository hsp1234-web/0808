...
const MOCK_VIDEO_URL =
  "https://www.youtube.com/watch?v=mock_video_for_validation";
...
test.describe("前端新功能驗證", () => {
  ...
  test.beforeEach(async ({ page }) => {
    // V2 REFACTOR: Navigate directly to the correct page.
    await page.goto("/youtube_report_v2.html", { waitUntil: "networkidle" });
    await expect(page.locator("h1")).toContainText("鳳凰音訊轉錄儀");
  ...
  test("YouTube下載：自訂命名、獨立進度條與報告預覽彈窗", async ({ page }) => {
    ...
    const ongoingTask = page.locator(
      `#ongoing-tasks .task-item:has-text("${MOCK_VIDEO_URL}")`,
    );
