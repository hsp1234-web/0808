...
test.describe("Main Validation Suite", () => {
  test("should successfully process a local audio file after fixing dependencies", async ({
    page,
  }) => {
    ...
    const audioFilePath = path.join(__dirname, "fixtures", "test_audio_v2.mp3");
    await page.locator('input[type="file"]').setInputFiles(audioFilePath);

    // Wait for the file to be recognized by the UI
    await expect(page.locator(".file-info-name")).toContainText(
      "test_audio_v2.mp3",
    );
    ...
    await page.screenshot({
      path: "test-results/e2e_main_validation_success.jpg",
      fullPage: true,
    });
  });
});
