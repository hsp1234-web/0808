...
test.describe.skip("全面 UI 功能驗證", () => {
  ...
  test.beforeEach(async ({ page }) => {
    // V2 REFACTOR: Removed hard-coded SERVER_URL.
    // Playwright's baseURL from the config will be used instead.
  ...
  test("媒體下載器應能正確響應 UI 互動", async ({ page }) => {
    ...
    // 測試輸入並開始下載
    await page
      .locator("#downloader-urls-input")
      .fill("https://youtube.com/shorts/eIAg2P5rHmw");
    ...
    await expect(downloaderTasks.locator(".task-filename")).toContainText(
      "https://youtube.com/shorts/eIAg2P5rHmw",
    );
  });
