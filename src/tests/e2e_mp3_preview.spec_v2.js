...
test.describe("MP3 預覽功能 E2E 測試", () => {
  ...
  test("使用者點擊預覽後，應能在彈出視窗看到音訊播放器並載入音訊", async ({
    page,
    context,
  }) => {
    // 1. V2 REFACTOR: Navigate directly to the downloader page
    ...
    const completedTask = page.locator("#downloader-tasks .task-item", {
      hasText: /e2e_test_video/,
    });
    ...
    page.on("response", (response) => {
      if (response.url().includes("/media/")) {
        console.log(
          `攔截到媒體請求: ${response.url()} - 狀態: ${response.status()}`,
        );
    ...
    await expect(previewModal.locator("h2")).toHaveText(
      `預覽: '${testUrl}' 的模擬影片標題`,
    );
    ...
    await expect(modalBody).toHaveAttribute(
      "data-last-preview-url",
      /^\/media\/e2e_test_.*\.mp3$/,
    );
    ...
    expect(mediaResponseStatus).toBeLessThan(
      400,
