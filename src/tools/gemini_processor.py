...
def generate_content_with_timeout(model, prompt_parts: list, log_message: str, internal_timeout: int = 100):
    """使用 ThreadPoolExecutor 包裝 model.generate_content 以實現強制超時。"""
    log.info(f"正要呼叫 model.generate_content ({log_message})...")
    external_timeout = internal_timeout + 10

    def generation_task():
        try:
            response = model.generate_content(prompt_parts, request_options={'timeout': internal_timeout})
            return response
        except Exception as e:
...
def main():
    ...
    if args.command == "list_models":
        ...
        if not audio_path.exists():
            ...
            sys.exit(1)
        try:
            process_audio_file(audio_path=audio_path, model=process_args.model, video_title=process_args.video_title, output_dir=Path(process_args.output_dir), tasks=process_args.tasks, output_format=process_args.output_format)
        except Exception as e:
