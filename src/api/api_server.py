...
# --- 廣播佇列 ---
# JULES'S FIX (2025-09-02): 使用執行緒安全的佇列來取代 run_coroutine_threadsafe
# 這是解決執行緒與 asyncio 事件迴圈之間同步問題的穩健作法。
broadcast_queue = queue.Queue()

async def broadcast_consumer():
    """一個非同步的消費者，從佇列中取出訊息並廣播。"""
    log.info("📢 廣播消費者已啟動，等待訊息...")
    loop = asyncio.get_running_loop()
    while True:
        try:
            # 在執行器中執行阻塞的 queue.get()，避免阻塞事件迴圈
            message = await loop.run_in_executor(None, broadcast_queue.get)
            if message is None: # 使用 None 作為信號來停止消費者
                log.info("📢 廣播消費者收到停止信號。")
                break
            await manager.broadcast_json(message)
            broadcast_queue.task_done()
        except Exception as e:
            log.error(f"📢 廣播消費者發生錯誤: {e}", exc_info=True)
...
async def lifespan(app: FastAPI):
    ...
    # 在應用程式關閉時執行的程式碼
    log.info("應用程式正在關閉，正在停止廣播消費者...")
    broadcast_queue.put(None) # 發送停止信號
    await consumer_task # 等待消費者處理完畢
    log.info("✅ 廣播消費者已成功停止。")
...
def trigger_model_download(model_size: str):
    """
    在一個單獨的執行緒中執行模型下載，並透過佇列將結果傳回主事件迴圈進行廣播。
    """
...
def trigger_transcription(task_id: str, file_path: str, model_size: str, language: Optional[str], beam_size: int, original_filename: Optional[str] = None):
    """
    在一個單獨的執行緒中執行轉錄，並透過佇列將結果傳回主事件迴圈進行廣播。
    """
    def _transcribe_in_thread():
        ...
        try:
            ...
            broadcast_queue.put(start_message)
            ...
            if process.returncode == 0:
                ...
                final_result_obj = {
                    ...
                    "output_path": convert_to_media_url(str(output_file_path))
                }
                ...
                stderr_output_msg = stderr_output or "N/A"
                log.error(f"❌ [執行緒] 轉錄任務 '{task_id}' 失敗。返回碼: {process.returncode}。Stderr: {stderr_output_msg}")
                final_message = {
                    "type": "TRANSCRIPTION_STATUS",
                    "payload": {"task_id": task_id, "status": "failed", "error": stderr_output_msg}
                }

            log.info(f"✅ [執行緒] 準備將最終訊息放入佇列: {json.dumps(final_message)}")
            broadcast_queue.put(final_message)
            ...
            broadcast_queue.put(error_message)

    thread = threading.Thread(target=_transcribe_in_thread)
    thread.start()


def trigger_youtube_processing(task_id: str):
    """在一個單獨的執行緒中執行 YouTube 處理流程，並透過佇列回報結果。"""
    def _process_in_thread():
        ...
        try:
            ...
            log.info(f"✅ [執行緒] 準備將最終訊息放入佇列: {json.dumps(update_message)}")
            broadcast_queue.put(update_message)
            log.info(f"✅ [執行緒] Gemini AI 任務完成訊息已放入佇列。")
            ...
            try:
                ...
                if msg_type == "DOWNLOAD_MODEL":
                    ...
                    if model_size:
                        ...
                        trigger_model_download(model_size)
                    else:
                    ...
                    if not file_path:
                        ...
                        trigger_transcription(task_id, file_path, model_size, language, beam_size, original_filename=original_filename)
                    ...
                    trigger_youtube_processing(task_id)
